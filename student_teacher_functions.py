...
def resample1_RED(in1, in2, filter, factor, *args, **kwargs):
    # up and up
    #branch 1
    C01 = Conv2D(use_bias=False, filters = filter[0], kernel_size= (1,1))
    ...
    C02 = Conv2D(use_bias=False, filters = filter[1], kernel_size= (1,1))
    ...
    #branch2
    C11 = Conv2D(use_bias=False, filters = filter[2], kernel_size= (1,1))
    ...
    C12 = Conv2D(use_bias=False, filters = filter[3], kernel_size= (1,1))
...
def resample2_RED(in1, in2, filter, factor, *args, **kwargs):
    #down and down
    #branch 1
    C01 = Conv2D(use_bias=False, filters = filter[0], kernel_size= (1,1))
    # C01.set_weights(Cwt[0])
    ...
    C02 = Conv2D(use_bias=False, filters = filter[1], kernel_size= (3,3), strides=(2,2), padding = "same")
    # C02.set_weights(Cwt[1])
    BN02 = BatchNormalization()
    # BN02.beta = BN_beta[1]
    # BN02.gamma = BN_gamma[1]
    ...
    C03 = Conv2D(use_bias=False, filters = filter[2], kernel_size= (1,1))
    ...
    #branch 2
    C11 = Conv2D(use_bias=False, filters = filter[3], kernel_size= (1,1))
    ...
    C12 = Conv2D(use_bias=False, filters = filter[4], kernel_size= (3,3), strides=(2,2), padding = 'same')
    ...
    C13 = Conv2D(use_bias=False, filters = filter[5], kernel_size= (1,1))
...
def resample3_RED(in1, in2, filter, factor, *args, **kwargs):
    #up and down
    ...
    C12 = Conv2D(use_bias=False, filters = filter[3], kernel_size= (3,3), strides=(2,2), padding = 'same')
    # C12.set_weights(Cwt[3])
    BN12 = BatchNormalization()
    # BN12.beta = BN_beta[3]
    # BN12.gamma = BN_gamma[3]
    ...
    C13 = Conv2D(use_bias=False, filters = filter[4], kernel_size= (1,1))
...
def input_img_RED(in1, Cwt, BN_beta, BN_gamma, BN_var, BN_mean):
...
def output_RED(inp, filter):

    C01 = Conv2D(use_bias=False, filters = filter, kernel_size= (1,1))
    # C01.set_weights(Cwt)
    BN01 = BatchNormalization()
    # BN01.beta = BN_beta
    # BN01.gamma = BN_gamma
...
def residual_RED(in1, filter):

    C1 = Conv2D(use_bias=False, filters = filter[0], kernel_size= (3,3), padding = "same")
    ...
    C2 = Conv2D(use_bias=False, filters = filter[1], kernel_size= (3,3), padding = "same")
...
def bottleneck_RED(in1, filter):

    C1 = Conv2D(use_bias=False, filters = filter[0], kernel_size= (1,1))
    # C1.set_weights(Cwt[0])
    BN1 = BatchNormalization()
    # BN1.beta = BN_beta[0]
    # BN1.gamma = BN_gamma[0]
    ...
    C3 = Conv2D(use_bias=False, filters = filter[2], kernel_size= (1,1))

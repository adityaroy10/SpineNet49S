...
def Student():
    ...
    x = input_img(inp, in0_conv, in0_beta, in0_gamma, in0_var, in0_mean)
    # sc = x
    ...
    in1 = resample2_RED(x01, x02, [ALPHA*164, ALPHA*164, 166, ALPHA*164, ALPHA*164, 166], [2,2])
    x1 = residual_RED(in1, [166, 166])

    in2 = resample2_RED(x01, x1, [ALPHA*164, ALPHA*164, 664, ALPHA*166, ALPHA*166, 664], [3,1])
    x2 = bottleneck_RED(in2, [664, 166, 664])

    in3 = resample2_RED(x1, x2 , [ALPHA*166, ALPHA*166, 166, ALPHA*664, ALPHA*664, 166], [3,2])
    x3 = residual_RED(in3, [166, 166])

    #### 3rd input
    in4 = resample1_RED(x2, x3, [ALPHA*664 ,664, ALPHA*166, 664], [-1, 4])
    in4 = Add()([in4, in2])
    in4 = Activation("relu")(in4)
    x4 = bottleneck_RED(in4, [664, 166, 664])

    in5 = resample1_RED(x1, x4, [ALPHA*166, 332, ALPHA*664, 332], [2, 4])
    x5 = bottleneck_RED(in5, [332, 83, 332])

    in6 = resample2_RED(x1, x5, [ALPHA*166, ALPHA*166, 664, ALPHA*332, ALPHA*332, 664], [2, 3])
    x6 =  bottleneck_RED(in6, [664, 166, 664])

    o1 = output_RED(x4, 256)    #4x
    o2 = output_RED(x5, 256)    #0times
    o3 = output_RED(x6, 256)    #8x

    o1 = UpSampling2D(size = (4,4))(o1)
    o3 = UpSampling2D(size = (8,8))(o3)
    out = Add()([o1,o2,o3])
    model = keras.Model(inputs = inp, outputs = out, name = 'Student')
    return model
...
def Teacher():
    ...
    o5 = UpSampling2D(size = (8,8))(o5)
    o4 = UpSampling2D(size = (16,16))(o4)
    o3 = UpSampling2D(size = (4,4))(o3)
    o1 = UpSampling2D()((o1))

    out = Add()([o1,o2,o3,o4,o5])

    model = keras.Model(inputs = input, outputs = out, name = 'Teacher')
